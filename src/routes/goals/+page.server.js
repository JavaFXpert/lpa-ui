...
export async function load({fetch, params}) {
 try {
  const MISSION_URL = `
  https://life-planning-assistant.replit.app/fetch_personal_mission_statement`;
  const res_mission = await fetch(MISSION_URL);
  const mission = await res_mission.json();

  const VISION_URL = `
  https://life-planning-assistant.replit.app/fetch_personal_vision_statement`;
  const res_vision = await fetch(VISION_URL);
  const vision = await res_vision.json();

  const SWOT_URL = `
  https://life-planning-assistant.replit.app/fetch_personal_swot_analysis`;
  const res_swot = await fetch(SWOT_URL);
  const swot = await res_swot.json();

  const GOALS_URL = `
  https://life-planning-assistant.replit.app/fetch_goals_summary`;
