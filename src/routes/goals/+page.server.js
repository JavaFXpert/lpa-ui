...
export async function load({fetch, params}) {
 try {
  const MISSION_URL = `
  https://lpa.javafxpert.repl.co/fetch_personal_mission_statement`;
  const res_mission = await fetch(MISSION_URL);
  const mission = await res_mission.json();

  const VISION_URL = `
  https://lpa.javafxpert.repl.co/fetch_personal_vision_statement`;
  const res_vision = await fetch(VISION_URL);
  const vision = await res_vision.json();

  const SWOT_URL = `
  https://lpa.javafxpert.repl.co/fetch_personal_swot_analysis`;
